...
import com.krm.tmdb.utils.ConstFun.showPopupMenuExternal
import com.krm.tmdb.utils.ConstFun.getUrlYoutube
import com.krm.tmdb.utils.ConstFun.setImageUrl
import com.krm.tmdb.utils.ConstFun.showPopupMenu
import com.krm.tmdb.utils.ConstFun.openDrawOverPermissionSetting
import com.krm.tmdb.utils.ConstStrings
import com.krm.tmdb.data.model.*
import com.krm.tmdb.videoplayer.DemoQSVideoView
import com.krm.tmdb.videoplayer.IVideoPlayer
import com.krm.tmdb.videoplayer.PlayListener
import com.krm.tmdb.videoplayer.QSVideoView
...
class FragmentDetailsTop() : BaseFragment(), InterfaceFragmentDetails.View, PlayListener,
    ...
    constructor(parcel: Parcel) : this() {
        idDetails = parcel.readInt()
        image = parcel.readString()
        key = parcel.readString()
        name = parcel.readString()
        title = parcel.readString()
        checkImage = parcel.readByte() != 0.toByte()
        fb = parcel.readString()
        tw = parcel.readString()
        it = parcel.readString()
        tm = parcel.readString()
    ...
    override fun writeToParcel(parcel: Parcel, flags: Int) {
        parcel.writeInt(idDetails)
        parcel.writeString(image)
        parcel.writeString(key)
        parcel.writeString(name)
        parcel.writeString(title)
        parcel.writeByte(if (checkImage) 1 else 0)
    ...
    companion object CREATOR : Parcelable.Creator<FragmentDetailsTop> {
        override fun createFromParcel(parcel: Parcel): FragmentDetailsTop {
            return FragmentDetailsTop(parcel)
        }

        override fun newArray(size: Int): Array<FragmentDetailsTop?> {
